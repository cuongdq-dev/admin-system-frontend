...
export function ServerView() {
  ...
  const _servers = [] as ServerProps[];

  const [filterName, setFilterName] = useState('');
  const dataFiltered: ServerProps[] = applyFilter({
    inputData: _servers,
          ...
          <TableContainer sx={{ overflow: 'unset' }}>
            <Table sx={{ minWidth: 800 }}>
              <ServerTableHead
                ...
                rowCount={_servers.length}
                numSelected={table.selected.length}
                onSort={table.onSort}
                onSelectAllRows={(checked) =>
                  table.onSelectAllRows(
                    checked,
                    _servers.map((server) => server.id)
                  )
                }
                headLabel={[
                  { id: 'name', label: 'Name' },
              ...
              <TableBody>
                ...
                <TableEmptyRows
                  height={68}
                  emptyRows={emptyRows(table.page, table.rowsPerPage, _servers.length)}
                />
